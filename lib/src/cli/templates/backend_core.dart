  ...
  final spec = {
    for (final collection in schema.collections.values)
      collection.name: {
        ...
        if (collection.sync.mode != SyncMode.full)
          'sync': {
            'mode': collection.sync.mode.yamlName,
            if (collection.sync.field != null) 'field': collection.sync.field,
            if (collection.sync.window != null)
              'windowMs': collection.sync.window!.inMilliseconds,
          },
