...
Map<String, String> generateBackendFiles(OnebaseSchema schema) {
...
Generated by `dart run onebase:setup`. One tiny server, four routes, no
framework:
...
Point your Flutter app's `apiUrl` at wherever this ends up — onebase
...
```bash
docker build -t onebase-backend .
docker run -p 3000:3000 --env-file .env onebase-backend
```
...
- Only fields declared in `onebase.yaml` are written — a patched client
...
- File bytes never pass through this server: it signs short-lived URLs and the
  ...
  approved.
- Deletes are recorded in `_onebase_tombstones` (TTL: 30 days) so other
