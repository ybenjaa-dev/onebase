...
Future<List<Diagnosis>> diagnose({
  ...
  String schemaFile = 'mongo_easy.yaml',
  String? apiUrl,
  ...
  if (!schemaSource.existsSync()) {
    return [
      Diagnosis(
        ...
        fix: 'dart run mongo_easy:setup --init',
      ),
    ];
  }
  ...
  // --- generated Dart ------------------------------------------------------
  final generated = File(p.join(root, 'lib', 'mongo_easy_schema.g.dart'));
  if (!generated.existsSync()) {
    findings.add(const Diagnosis(
      DiagnosisLevel.error,
      'lib/mongo_easy_schema.g.dart is missing',
      fix: 'dart run mongo_easy:setup',
    ));
  } else if (generated.readAsStringSync().trim() !=
      generateDartSchema(schema).trim()) {
    findings.add(const Diagnosis(
      DiagnosisLevel.error,
      'lib/mongo_easy_schema.g.dart is out of date',
      detail: 'It no longer matches mongo_easy.yaml, so your models and the '
          'runtime schema disagree.',
      fix: 'dart run mongo_easy:setup --force',
    ));
  } else {
    findings.add(const Diagnosis(
  ...
  if (!core.existsSync()) {
    ...
    if (!source.contains(expected)) {
      findings.add(const Diagnosis(
        ...
        fix: 'dart run mongo_easy:setup --force, then redeploy backend/',
      ));
    } else {
      findings.add(const Diagnosis(
...
Future<Diagnosis> _checkApi(
  ...
  try {
    ...
    return Diagnosis(
      ...
      fix: 'Check that apiUrl points at the mongo_easy backend itself.',
    );
