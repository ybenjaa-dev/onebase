...
import 'package:mongo_easy/src/cli/doctor.dart';
...
void main() {
  ...
  setUp(() => root = Directory.systemTemp.createTempSync('mongo_easy_doctor'));
  ...
  void scaffold({String yaml = _yaml}) {
    write('mongo_easy.yaml', yaml);
    final schema = parseSchemaYaml(yaml);
    write('lib/mongo_easy_schema.g.dart', generateDartSchema(schema));
  ...
  test('reports a schema that does not parse', () async {
    write('mongo_easy.yaml', 'collections: []');
    final findings = await diagnose(root: root.path);
    expect(findings.single.level, DiagnosisLevel.error);
...
''');
  ...
  group('drift', () {
    test('catches a stale generated Dart schema', () async {
      scaffold();
      write('lib/mongo_easy_schema.g.dart', '// stale');
      final findings = await diagnose(root: root.path);
...
''');
      final schema = parseSchemaYaml(
          File(p.join(root.path, 'mongo_easy.yaml')).readAsStringSync());
      write('lib/mongo_easy_schema.g.dart', generateDartSchema(schema));
    ...
    test('reports a missing backend', () async {
      write('mongo_easy.yaml', _yaml);
      write('lib/mongo_easy_schema.g.dart',
